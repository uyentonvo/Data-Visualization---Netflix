...
// Parse the Data
Promise.all([
  d3.csv("https://raw.githubusercontent.com/casihoicho/DSDV-repo/Duy/data/rating1%20(1).csv"),
...
var bar=svg.append("g")
  ...
  .selectAll("rect")
  .data(function(d) { return subgroups.map(key => ({key, value: d[key]})); })
  .join("rect")
    .attr("x", x(0))
...
let group = output.map(d => d.key);
        svg2.selectAll("*").remove();
          ...
          .domain([0, d3.max(output, function(d) { return d.value; })])
          .range([ 0, width]);
        svg2.append("g")
          .attr("transform", `translate(0, ${height})`)
      ...
      .data(output)
        // .data(function(d) { return dat.map(key => ({key, value: d[key]})); })
        .enter()

        .append("line")
          .attr("x1", function(d) { return x0(d.value); })
          .attr("x2", x0(0))
          .attr("y1", function(d) { return y0(d.key); })
          .attr("y2", function(d) { return y0(d.key); })
          .attr("stroke", "grey")
        .transition()
        .duration(1000);
